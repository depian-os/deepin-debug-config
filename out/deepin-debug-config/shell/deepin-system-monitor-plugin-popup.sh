...
app_id="org.deepin.system-monitor"
configuration_id="org.deepin.system-monitor.plugin.popup"
...
case "${debug}" in
"debug")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*.debug=true;*.info=false;*.warning=false" #按需改变分类、添加子模块规则
	exit 0
	;;
"info")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*.debug=false;*.info=true;*.warning=false" #按需改变分类、添加子模块规则
	exit 0
	;;
"warning")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*.debug=false;*.info=false;*.warning=true" #按需改变分类、添加子模块规则
	exit 0
	;;
esac
