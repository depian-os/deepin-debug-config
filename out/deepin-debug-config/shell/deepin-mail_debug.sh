...
app_id="org.deepin.mail"
configuration_id="org.deepin.mail"
...
case "${debug}" in
"debug")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*mail*.debug=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"info")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*mail*.debug=false;*mail*.info=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"warning")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*mail*.debug=false;*mail*.info=false;*mail*.warning=true" #按需改变分类、添加子模块规则
	exit 0
	;;
esac
