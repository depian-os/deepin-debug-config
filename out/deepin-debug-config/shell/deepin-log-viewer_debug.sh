...
app_id="org.deepin.log.viewer"
configuration_id="org.deepin.log.viewer"
...
case "${debug}" in
"debug")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*log.viewer*.debug=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"info")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*.debug=false;*log.viewer*.info=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"warning")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*.debug=false;*.info=false;*log.viewer*.warning=true" #按需改变分类、添加子模块规则
	exit 0
	;;
esac
