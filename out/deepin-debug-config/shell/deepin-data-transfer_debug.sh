...
# 默认日志级别
log_level="info"
config_file="/etc/deepin-data-transfer/config/log.conf"
daemon_config_file="/etc/dde-cooperation-daemon/config/log.conf"

# 检查目录是否存在，如果不存在则创建
if [ ! -d "/etc/deepin-data-transfer/config" ]; then
    mkdir -p /etc/deepin-data-transfer/config
fi
...
echo "deepin-data-transfer log level is $(grep 'g_minLogLevel' $config_file)"
