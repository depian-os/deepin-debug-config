...
app_id="org.deepin.terminal"
configuration_id="org.deepin.terminal"
...
case "${debug}" in
"debug")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*terminal*.debug=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"info")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*terminal*.debug=false;*terminal*.info=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"warning")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*terminal*.debug=false;*terminal*.info=false;*terminal*.warning=true" #按需改变分类、添加子模块规则
	exit 0
	;;
esac
