...
configuration_id="org.deepin.dde.file-manager"
...
case "${debug}" in
"debug")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*filemanager*.debug=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"info")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*.debug=false;*filemanager*.info=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"warning")
	dde-dconfig --set -a $app_id -r $configuration_id -k log_rules -v "*.debug=false;*.info=false;*filemanager*.warning=true" #按需改变分类、添加子模块规则
	exit 0
	;;
esac
