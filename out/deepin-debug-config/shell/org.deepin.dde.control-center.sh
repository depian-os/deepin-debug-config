...
app_id="org.deepin.dde.control-center"
...
case "${debug}" in
"debug")
	dde-dconfig --set -a $app_id -r $configuration_id -k rules -v "*.debug=true;qt.*.debug=false" #按需改变分类、添加子模块规则
	exit 0
	;;
"info")
	dde-dconfig --set -a $app_id -r $configuration_id -k rules -v "*.debug=false;*.info=true" #按需改变分类、添加子模块规则
	exit 0
	;;
"warning")
...
if [ "$state" == "y" ]; then
  rules=$(dde-dconfig --get -a $app_id -r $configuration_id -k rules) # 规则最后一位必须为true,默认全局warning为true
